...
A = rnorm(100)
plot(A, A + runif(100,min=-.2,max=.2))

points(A[1:90] + runif(90,min=-.2,max=.2), A[1:90] + runif(90,min=-.2,max=.2), col = 2)


# the relationsship of edge length and MIR L2 for each species
# that way we get a sense of the underlying distribtuion of TEs and how it changes
# if we just standardise the differences it should be all good
# and select from the standardised differences
# or maybe the PCA mehtod
